...
Therefore, immediately after the fieldset we have the length in byte of the balance: `0x48` (the length would be `0x48` - `0x40` = `8` bytes). the following 8 bytes (aka. 16 nibbles) contains the hexadecimal value of the balance that in the first record would be: `0x7ce66c50e2840000`.
````
$ python
Python 2.7.15
...
What follows are excerpts from the turbo-geth's database after block 1 has been inserted. These excerpts only show the records that have changed, and
...
The remaining 32 bytes of the key encode the block hash. The value is the RLP-encoded list of receipts. In our case, there a transaction was sent and as a matter of fact, instead of having `0xC0` (that repressent an empty list) we have `0xc6c501825208c0` that his the receipt for the 0.001 ETH transaction.
...
* The first record has a field set of `0x3` (`11`) meaning that this time the accound has a nonce as well. since the nonce is less than `24` it has no length byte. therefore the nonce is stored in the byte immediately after the field set: `0x01`. the balance is now changed to `0x7ce2ded23dbd8000`.
...
1000000000000000
````
Which is 0.01 ETH that is exactly how much has been sent to the recipient address, meaning that this account repressent the recipient of the transaction and the incarnation is set to `1`.

In the third bit is contained Incarnation. The Incarnation is a turbo-geth specific attribute, which is
